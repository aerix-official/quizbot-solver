...
[![Platform](https://img.shields.io/badge/platform-Chrome-orange?style=flat-square)](https://chrome.google.com/webstore)

*A powerful Chrome extension that automatically solves **McGraw-Hill SmartBook** assignments using Claude API or free AI tabs.*

[Features](#features) • [Installation](#installation) • [AI Modes](#ai-modes) • [Setup](#setup) • [Usage](#usage)
...
5. The QuizBot icon will now appear in your toolbar!
...
2. Click the **QuizBot Icon** → **Settings ⚙**.
3. Paste your key (starts with `sk-ant-`) and click **Save Key**.
4. Set the mode to **Claude** in the main popup.

### 2. Tab AI Mode (Free)
1. Open [ChatGPT](https://chatgpt.com), [Gemini](https://gemini.google.com), or [DeepSeek](https://chat.deepseek.com) and log in.
...
### Popup Interface
- **Confidence Pills:** Select High, Medium, or Low default confidence.
- **Randomize Confidence:** Toggles random confidence selection for each question.
- **Double Credit Mode:** Automated tab duplication for double completion.
- **Pause Before Submit:** Fills the answer but waits for your manual click.

### API Configuration
- **Model Selection:** Choose between **Claude Sonnet** (Fast) or **Claude Opus** (Highest Accuracy).
- **Max Tokens:** Limit output to save API costs (1024 recommended).
